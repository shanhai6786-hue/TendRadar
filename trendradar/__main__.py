...
class NewsAnalyzer:
    ...
    def _crawl_rss_data(self) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]]]:
        ...
        try:
            ...
            opml_file = rss_config.get("OPML_FILE")

            if opml_file:
                from trendradar.crawler.rss.opml_parser import parse_opml
                import os

                if not os.path.isabs(opml_file):
                    config_dir = os.path.dirname(self.ctx.config.get("CONFIG_PATH", ""))
                    if not config_dir:
                        config_dir = "config"
                    opml_file = os.path.join(config_dir, opml_file)

                try:
                    opml_feeds = parse_opml(opml_file)
                    rss_feeds = opml_feeds
                    print(f"[RSS] 从 OPML 文件加载了 {len(rss_feeds)} 个源")
                except Exception as e:
                    print(f"[RSS] OPML 解析失败: {e}，使用配置文件中的 feeds")
