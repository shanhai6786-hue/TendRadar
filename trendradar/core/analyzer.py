...
def count_word_frequency(
    ...
    if platform_configs:
        for platform in platform_configs:
            ...
            enable_filter = platform.get("enable_keyword_filter", True)
            if platform_id:
                platform_filter_map[platform_id] = enable_filter
    ...
    for source_id, titles_data in results_to_process.items():
        ...
        for title, title_data in titles_data.items():
            ...
            # 检查该平台是否启用关键词过滤
            enable_filter = platform_filter_map.get(source_id, True)

            # 如果平台禁用了关键词过滤，直接匹配到"全部新闻"词组
