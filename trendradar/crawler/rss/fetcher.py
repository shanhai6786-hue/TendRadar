...
class RSSFetcher:
    ...
    def from_config(cls, config: Dict) -> "RSSFetcher":
        ...
        # 优先使用 OPML 文件
        opml_file = config.get("opml_file")
        if opml_file:
            if not os.path.isabs(opml_file):
                config_dir = os.path.dirname(os.path.abspath(__file__))
                opml_file = os.path.join(config_dir, "../../../config", opml_file)

            try:
                opml_feeds = parse_opml(opml_file)
                for feed_data in opml_feeds:
                    feed = RSSFeedConfig(
                        id=feed_data["id"],
                        name=feed_data["name"],
                        url=feed_data["url"],
                    ...
                    feeds.append(feed)
                print(f"[RSS] 从 OPML 文件加载了 {len(feeds)} 个源")
            except Exception as e:
                print(f"[错误] 无法解析 OPML 文件: {e}")

        # 如果没有 OPML 或解析失败，使用 feeds 列表
        if not feeds:
