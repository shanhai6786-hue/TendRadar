...
def generate_html_report(
    ...
    # 复制 CSS 文件到 output/html/static/
    css_source = Path(__file__).parent / "static" / "report.css"
    css_dest_dir = Path(output_dir) / "html" / "static"
    css_dest_dir.mkdir(parents=True, exist_ok=True)
    if css_source.exists():
        shutil.copy2(css_source, css_dest_dir / "report.css")
